...
public class FuzzyStringMatch {
  ...
  public static boolean fuzzyStringValid(
      String inspected, String lookingFor, int percentageRequired)
      throws FuzzyStringMatchException {
    return (percentageRequired) <= fuzzyStringContains(inspected, lookingFor);
  }

  public static int fuzzyStringContains(String inspected, String lookingFor)
      throws FuzzyStringMatchException {
    if (!isValidString(inspected)) {
      throw new FuzzyStringMatchException(
          String.format("invalid input strings : inspected : '%s'", inspected));
    }
    if (!isValidString(lookingFor)) {
      throw new FuzzyStringMatchException(
          String.format("invalid input strings : lookingFor : '%s'", lookingFor));
    }

    String localLookingFor =
        !lookingFor.matches("^[A-Za-z .,;:'0-9]+")
            ? replaceInvalidCharacters(lookingFor).toUpperCase()
            : lookingFor.toUpperCase();
    String localInspected =
        !inspected.matches("^[A-Za-z .,;:'0-9]+")
            ? replaceInvalidCharacters(inspected).toUpperCase()
            : inspected.toUpperCase();
    int returnValue = 0;
    int charIndex = 0;

    while (charIndex + localLookingFor.length() <= localInspected.length()
        && returnValue < PERFECT) {
      String workingString =
          localInspected.substring(charIndex, charIndex + localLookingFor.length());

      int matchValue =
          workingString.equals(localLookingFor)
              ? PERFECT
              : fuzzyStringsMatch(workingString, localLookingFor);
      if (matchValue > returnValue) {
        returnValue = matchValue;
      }

      // just to show the string traversing the inspected list
      LOGGER.debug(">{}< :: >{}< = {}", localLookingFor, workingString, matchValue);
  ...
  protected static int fuzzyStringsMatch(String inspected, String lookingFor)
      throws FuzzyStringMatchException {
    int returnValue = 0;
  ...
  protected static String replaceInvalidCharacters(String inspected)
      throws FuzzyStringMatchException {
    ...
    for (char letter : inspectedArray) {
      ...
      if (!tempLetter.matches("^[A-Za-z .,;:'0-9]")) {
        ...
        localInspected =
            localInspected.substring(0, count)
                + replacingWith
                + localInspected.substring(
                    count + 1, localInspected.length() - replacingWith.length() + 1);
      }
      count++;
    }

...
@CucumberOptions(plugin = "json:target/cucumber-report.json", tags = {"@FitnoteDeclarationTest"}) // only run declaration tests to test encryption/decryption
public class RunEncryptedCukesTest {
    private static LocalstackDocker localstackDocker = LocalstackDocker.INSTANCE;

    static {
        LocalstackDockerConfiguration dockerConfig = new LocalstackDockerAnnotationProcessor().process(RunEncryptedCukesTest.class);
    ...
    @ClassRule
    public static final DropwizardAppRule<FitnoteControllerConfiguration> RULE = new DropwizardAppRule<>(FitnoteControllerApplication.class, resourceFilePath("test-crypto.yml"));
}

Feature: Fitnote declaration
    ...
    Scenario: Submit an accepted declaration with all mandatory session data present
      ...
      And I create a catch all subscription for queue name "test.queue.1" binding to exchange "rabbit.fitnote.ex"
      When I hit the service url "http://localhost:9100/photo" with the following json body
        | sessionId         | "11"                 |
        | image             | /OcrTest.jpg        |
      Then I receive a HTTP response of 202
      And I hit the service url "http://localhost:9100/imagestatus" with session id "11" getting return status 200 and finally containing the following json body
      ...
      When I hit the service url "http://localhost:9100/address" with the following json body
        | sessionId         | "11"              |
        | houseNameOrNumber | "254"             |
        | postcode          | "NE12 9PG"        |
      Then I receive a HTTP response of 200
      When I hit the service url "http://localhost:9100/declaration" with the following json body
      | sessionId      | "11" |
      | accepted       | true |
      Then I receive a HTTP response of 200
  ...
  Scenario: Submit a declined declaration with all mandatory session data present
    Given the controller is up
    And I create a rabbit exchange named "rabbit.fitnote.ex"
    And I create a catch all subscription for queue name "test.queue.2" binding to exchange "rabbit.fitnote.ex"
    When I hit the service url "http://localhost:9100/photo" with the following json body
      | sessionId         | "12"                 |
      | image             | /OcrTest.jpg        |
    Then I receive a HTTP response of 202
  ...
  Scenario: Submit an accepted declaration with session missing fitnote image
    Given the controller is up
    And I create a rabbit exchange named "rabbit.fitnote.ex"
    And I create a catch all subscription for queue name "test.queue.3" binding to exchange "rabbit.fitnote.ex"
    When I hit the service url "http://localhost:9100/nino" with the following json body
      | sessionId         | "13"                 |
      | nino              | "AA370773"          |
    Then I receive a HTTP response of 200
  ...
  Scenario: Submit an accepted declaration with session missing an address
    Given the controller is up
    And I create a rabbit exchange named "rabbit.fitnote.ex"
    And I create a catch all subscription for queue name "test.queue.4" binding to exchange "rabbit.fitnote.ex"
    When I hit the service url "http://localhost:9100/photo" with the following json body
      | sessionId         | "14"                 |
      | image             | /OcrTest.jpg        |
    Then I receive a HTTP response of 202
  ...
  Scenario: Submit an accepted declaration with session missing NINO
    Given the controller is up
    And I create a rabbit exchange named "rabbit.fitnote.ex"
    And I create a catch all subscription for queue name "test.queue.5" binding to exchange "rabbit.fitnote.ex"
    When I hit the service url "http://localhost:9100/photo" with the following json body
      | sessionId         | "16"                 |
      | image             | /OcrTest.jpg        |
    Then I receive a HTTP response of 202
  ...
  Scenario: Submit invalid json as declaration
    Given the controller is up
    And I create a rabbit exchange named "rabbit.fitnote.ex"
    And I create a catch all subscription for queue name "test.queue.6" binding to exchange "rabbit.fitnote.ex"
    When I hit the service url "http://localhost:9100/photo" with the following json body
      | sessionId         | "17"                 |
      | image             | /OcrTest.jpg        |
    Then I receive a HTTP response of 202
    ...
    And I hit the service url "http://localhost:9100/imagestatus" with session id "17" getting return status 200 and finally containing the following json body
      | fitnoteStatus | SUCCEEDED |
  ...
  Scenario: Submit multiple accepted declarations with all mandatory session data present
    Given the controller is up
    And I create a rabbit exchange named "rabbit.fitnote.ex"
    And I create a catch all subscription for queue name "test.queue.7" binding to exchange "rabbit.fitnote.ex"
    When I hit the service url "http://localhost:9100/photo" with the following json body
      | sessionId         | "22"                 |
      | image             | /OcrTest.jpg        |
    Then I receive a HTTP response of 202
    ...
    When I hit the service url "http://localhost:9100/address" with the following json body
      ...
      | postcode          | "LS6 9PG"        |
    Then I receive a HTTP response of 200
    And I hit the service url "http://localhost:9100/imagestatus" with session id "23" getting return status 200 and finally containing the following json body
      | fitnoteStatus | SUCCEEDED |
    When I hit the service url "http://localhost:9100/declaration" with the following json body
      | sessionId      | "23" |
      | accepted       | true |
    Then I receive a HTTP response of 200
    And I hit the service url "http://localhost:9100/imagestatus" with session id "22" getting return status 200 and finally containing the following json body
      | fitnoteStatus | SUCCEEDED |
    When I hit the service url "http://localhost:9100/declaration" with the following json body
      | sessionId      | "22" |
      | accepted       | true |
